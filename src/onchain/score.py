...
class Verdict:
    ...
    def add(self, points: int, label: str) -> None:
        self.flags.append(Flag(points, label))
        self.score += points
...
def evaluate(token: TokenInfo, launch: LaunchWindow | None,
    ...
    if token.owner is not None and token.owner_renounced is False:
        v.add(1, f"owner not renounced ({token.owner})")
    if token.is_proxy:
        v.add(2, "upgradeable proxy — logic can be swapped after you buy")
    if token.source_verified is False:
        v.add(2, "contract source not verified")
    ...
    if clusters:
        ...
        if clusters.funding_clusters:
            ...
            v.add(3, f"{len(biggest[1])} early buyers share funder {biggest[0]} "
                     f"({len(clusters.funding_clusters)} funding cluster(s) total)")
    ...
    if holders:
        if holders.top10_pct >= 50:
            v.add(3, f"top 10 holders control {holders.top10_pct:.1f}% of circulating supply")
        elif holders.top10_pct >= 30:
            v.add(1, f"top 10 holders control {holders.top10_pct:.1f}%")
        if holders.holder_count < 100:
            v.add(1, f"only {holders.holder_count} holders")
